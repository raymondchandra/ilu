...
class ProductsController extends \BaseController {
	...
	{			
		...
		{
			$product = $product->where('promotion_id', '=', $input['promotion_id']);
		}		
		if($input['id'] != -1)
		{
		...
		if($input['category_name'] != "")
		{
			foreach($product as $key)
			{
				$pos = strpos($key->category_name, $input['category_name']);
				if($pos !== false)
				{
					$result[] = $key;
				}
